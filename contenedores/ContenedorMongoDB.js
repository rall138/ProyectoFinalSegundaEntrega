...
class ContenedorMongoDB {
    ...
    constructor(connectionURI, model){
        ...
        mongoose.connect(connectionURI, {useNewUrlParser: true, useUnifiedTopology: true}, err => {
            if (err){throw new Error(`Error en la conexión de la base de datos: ${err}`)}
        })
    ...
    async getById(id){
        let item = await this.model.findOne({id: id})
        return item
    }

    async update(id, item){
        let transactionObj = await this.model.findOneAndUpdate({id: id}, item)
        return transactionObj
    }

    async delete(id){
        let transactionObj = await this.model.deleteOne({id: id})
        return transactionObj
    }
